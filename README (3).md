# 🚀 100 Days of LeetCode — #DrGViswanathanChallenge

> Solving LeetCode problems every day for 100 days to sharpen DSA skills and build consistency.

![GitHub last commit](https://img.shields.io/github/last-commit/YourUsername/LeetCode-Solutions)
![Problems Solved](https://img.shields.io/badge/Problems%20Solved-2-brightgreen)
![Days Completed](https://img.shields.io/badge/Days%20Completed-1%2F100-blue)
![Language](https://img.shields.io/badge/Language-C%2B%2B-orange)

---

## 👨‍💻 About Me

- **Name:** Suryanshsky17
- **Challenge:** #DrGViswanathanChallenge — 100 Days of LeetCode
- **Language:** C++
- **Start Date:** June 06, 2026
- **Goal:** Solve at least 1 LeetCode problem every day and document the approach

---

## 📊 Progress Tracker

| Day | Problem No. | Problem Name | Difficulty | Topics | Runtime | Solution |
|-----|-------------|--------------|------------|--------|---------|----------|
| 01  | 263 | [Ugly Number](https://leetcode.com/problems/ugly-number/) | 🟢 Easy | Math, Loop | 0ms ⚡ | [View](./Day01/263_Ugly_Number.cpp) |
| 01  | 258 | [Add Digits](https://leetcode.com/problems/add-digits/) | 🟢 Easy | Math, Loop | 0ms ⚡ | [View](./Day01/258_Add_Digits.cpp) |
| 02  | 319 | [Bulb Switcher](https://leetcode.com/problems/bulb-switcher/) | 🟡 Medium | Math | 0ms ⚡ | [View](./Day02/319_Bulb_Switcher.cpp) |
| 02  | 1232 | [Check If It Is a Straight Line](https://leetcode.com/problems/check-if-it-is-a-straight-line/) | 🟢 Easy | Math, Geometry | 0ms ⚡ | [View](./Day02/1232_Check_Straight_Line.cpp) |
| 03  | 50 | [Pow(x, n)](https://leetcode.com/problems/powx-n/) | 🟡 Medium | Math, Recursion | 0ms ⚡ | [View](./Day03/50_Pow_x_n.cpp) |
| 03  | 168 | [Excel Sheet Column Title](https://leetcode.com/problems/excel-sheet-column-title/) | 🟢 Easy | Math, String | 0ms ⚡ | [View](./Day03/168_Excel_Sheet_Column_Title.cpp) |
| 04  | 342 | [Power of Four](https://leetcode.com/problems/power-of-four/) | 🟢 Easy | Math, Loop | 0ms ⚡ | [View](./Day04/342_Power_of_Four.cpp) |
| 04  | - | [GCD / HCF - Euclidean Algorithm](./Day04/GCD_HCF_Euclidean_Algorithm.cpp) | 🟢 Concept | Math, GCD | - | [View](./Day04/GCD_HCF_Euclidean_Algorithm.cpp) |
| 05  | - | [Divisors of a Number](./Day05/Divisors_of_a_Number.cpp) | 🟢 Concept | Math, sqrt(n) | - | [View](./Day05/Divisors_of_a_Number.cpp) |
| 05  | - | [Sieve of Eratosthenes](./Day05/Prime_Sieve_of_Eratosthenes.cpp) | 🟢 Concept | Math, Primes | - | [View](./Day05/Prime_Sieve_of_Eratosthenes.cpp) |
| 06  | - | - | - | - | - | - |
| 07  | - | - | - | - | - | - |
| 08  | - | - | - | - | - | - |
| 09  | - | - | - | - | - | - |
| 10  | - | - | - | - | - | - |

> ✅ Update this table daily!

---

## 📁 Folder Structure

```
LeetCode-Solutions/
├── README.md
├── Day01/
│   ├── 263_Ugly_Number.cpp
│   └── 258_Add_Digits.cpp
├── Day02/
│   └── 319_Bulb_Switcher.cpp
    └── 1232_Check_Straight_Line.cpp
└── Day100/
    └── ...
```

---

## 🧠 Topics Covered

| Topic | Problems Solved |
|-------|----------------|
| Math | 10 |
| Arrays | 0 |
| Strings | 0 |
| Linked List | 0 |
| Trees | 0 |
| Dynamic Programming | 0 |

---

## 📈 Stats

| Metric | Count |
|--------|-------|
| ✅ Total Problems Solved | 10 |
| 🟢 Easy | 6 |
| 🟡 Medium | 4 |
| 🔴 Hard | 0 |
| ⚡ 0ms Solutions | 6 |

---

## 🔗 Connect With Me

- 💼 [LinkedIn](https://linkedin.com/in/YourProfile)
- 🐙 [GitHub](https://github.com/YourUsername)
- 💻 [LeetCode](https://leetcode.com/YourUsername)

---

## 📜 Challenge Rules

- ✅ Solve at least **1 problem per day**
- ✅ Push solution to GitHub **every day**
- ✅ Write the **approach** as a comment in the code
- ✅ Post daily update on **LinkedIn** with `#DrGViswanathanChallenge`
- ❌ No skipping days!

---

⭐ *If you find this helpful, consider starring the repo!*
